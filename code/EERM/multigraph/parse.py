from model import Base, Model, Model_Multi, GSTOPR
...
def parse_method_gstopr(args, dataset, n, c, d, device):
    if args.gnn == 'gcn':
        model = GSTOPR(args, n, c, d, 'gcn', device).to(device)
    elif args.gnn == 'sage':
        model = GSTOPR(args, n, c, d, 'sage', device).to(device)
    elif args.gnn == 'gat':
        model = GSTOPR(args, n, c, d, 'gat', device).to(device)
    elif args.gnn == 'gpr':
        model = GSTOPR(args, n, c, d, 'gpr', device).to(device)
    elif args.gnn == 'gcnii':
        model = GSTOPR(args, n, c, d, 'gcnii', device).to(device)
    else:
        raise ValueError('Invalid method')
    return model
